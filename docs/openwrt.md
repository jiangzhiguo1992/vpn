...
盒子直接跑 sing-box 官方 OpenWrt 包(procd + UCI),配置与桌面产物同源渲染
,一条 make 命令 部署、常驻免管。
...
```bash
make doctor    # 环境自检(Go 1.27+ / ssh / scp),全绿继续
make gen       # 生成全部产物(含 dist/sing-box-openwrt.json);清单缺失时按提示
```
...
| 2 装内核 | 无 `sing-box` 时自动安装(`apk add` / `opkg install`,含 auto_route 依赖 `ip-full`;`kmod-tun` 由包自动依赖);装后复检,仍不可用时报错并给出 `opkg install --force-reinstall` / `apk fix` 指引(包记录已存在时跳过补装) |
| 3 UCI/init 集成检查 | 确认官方源包形态(`sing-box.main` + `/etc/init.d/sing-box`,官方源包安装时自带);手动放的 release apk 无此集成,会在此明确报错(请改用官方源/第三方源 ipk) |
| 4 语法校验 | `sing-box check -c` 校验上传配置——失败即停,**不触碰现网**,并打印内核版本首行供对照(内核过旧或语法不兼容时按第 6 章升级) |
| 5 备份 | 现网 `/etc/sing-box/config.json`、`/etc/config/dhcp`、`/etc/config/sing-box` 留 `.bak`(**只保留首次部署前备份**,重跑不覆盖) |
| 6 落位 | 配置写入 `/etc/sing-box/config.json`(0600),随即删除 `/tmp` 上传件 |
| 7 启用 | UCI `sing-box.main.user='root'`(TUN 需 root 运行)+ `enabled='1'` + `/etc/init.d/sing-box enable` |
| 8 启动 | `/etc/init.d/sing-box restart` |
| 9 DNS 让位 | dnsmasq `port='0'`(仅禁 DNS,DHCP 保留;dnsmasq 停用时不被意外拉起)并重启,打印原端口供恢复 |
...
输出 `✓ OpenWrt 网关 sing-box 部署完成` 即完成;再按下节清单真机验证。
...
> 部署输出中打印的 dnsmasq 原端口恢复命令与上述第 2 步等价,可直接使用。
...
| DNS | dnsmasq 只留 DHCP,DNS 归 sing-box(架构见第 5 章) |
| 规则集下载 | legacy 形态(`rule_set[].download_detour: "proxy"`),经代理隧道下载并落盘缓存(内核兼容性见第 6 章) |
...
内核由部署脚本经官方源自动安装,opkg/apk 按盒子架构(arm/aarch64/x86_64 等)
选包,无需指定;主流架构官方源均有 sing-box 包,极小众/过老架构缺失时会
明确报错(见第 8 章 FAQ)。
...
| 二合一路由 | 同软路由,叠加其它服务(AP/NAS/Docker 等) | 接管不受影响;注意 tun 网段与叠加服务网段(如 Docker bridge)不冲突(见第 7 章坑 2) |

**旁路由验证要点**:设备把网关+DNS 指向盒子后,按第 2 章清单逐项验收;
访问盒子/主路由管理页(LuCI)走局域网直连规则,不受接管影响。

## 5 DNS 架构与防环

sing-box 作者明确建议不要把它放在 dnsmasq 后面(防泄漏,见 SagerNet/sing-box#3705):
dnsmasq 的 DNS 停用(`port='0'`,脚本 9/9),sing-box 经 auto_redirect 接管 53:
...
- 恢复 dnsmasq DNS 即回滚流程:见第 3 章(部署输出中打印的原端口命令同样可用)

**防环**:变体已内置 `route.auto_detect_interface`(出站接口自动检测,防自身流量
被吸入 TUN);仍出现"大面积超时/疑似自环"症状时,查 `logread | grep sing-box`,
并确认本地无 DNS 指向 127.0.0.1 的残留(本地 DNS 直连会被 TUN 捕获成环)。
...
- **升级内核**:官方源内 `opkg/apk upgrade sing-box` 即可(配置不受影响);
  ...
  提供,但无 init/UCI 集成,不推荐裸用——部署会死在第 2 章脚本第 3 步检查)
...
1. **auto_redirect 与既有 NAT/端口转发**:sing-box 向 fw4 插入的 nftables 规则
   ...
   盒子有端口映射需求时**必须真机验证**(见第 2 章清单);冲突时可临时关闭配置里
   `auto_redirect`(仅留 auto_route,语义弱一些但冲突面小)
...
4. **IPv6**:双栈接管;内网无 IPv6 无影响;想禁用 v6 代理时删配置里 address 的 v6 段即可。
...
| 症状 | 排查 |
|---|---|
| 部署脚本报 FATAL 检出 | `logread \| grep sing-box`:多为规则集首启下载失败(节点不可达)或 tun 创建失败(网段冲突/缺权限),见第 7 章坑 3 与坑 2 |
| 全部不通 | `logread \| grep sing-box` 查错误;先 `sing-box check -c /etc/sing-box/config.json` |
| 国内通、国外不通 | 隧道握手失败:节点变更/服务器不可达;`logread` 查 ERROR,换 auto 组节点或重跑部署(第 2 章) |
| 网页打不开但 ping 通 | DNS 链路问题:确认 dnsmasq 已让位(第 5 章)、53 被 sing-box 接管 |
| (旁路由形态)国外站不通/解析异常 | 设备 DNS 未指向盒子,DNS 查询走主路由可能被污染;把网关与 DNS 都指向盒子(见第 4 章拓扑) |
| 大面积超时/疑似自环 | 见第 5 章防环:查本地 DNS 残留与 sing-box 日志 |
